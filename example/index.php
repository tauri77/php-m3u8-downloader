...
if ( isset( $_POST ) && isset( $_POST['action'] ) ) {
	...
	if ( $_POST['action'] === 'analyze' ) {
		...
		<form action="" method="POST">
			<label for="display_url">URL:</label> <span><?php echo htmlentities( $_POST['url'] ); ?></span>
			<input type="hidden" name="url" id="url"
				   value="<?php echo htmlspecialchars( $_POST['url'], ENT_COMPAT ); ?>"/>
			<br>
			<br>
	...
	if ( $_POST['action'] === 'download' ) {
		...
		Tauri\M3u8Downloader\Downloader::$modeFile = 0777;
        Tauri\M3u8Downloader\Downloader::$modeFile = 0777;
